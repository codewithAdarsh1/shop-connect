...
export default function WidgetGeneratorPage() {
  ...
  const [config, setConfig] = useState({
    ...
    welcomeMessage: "Hey! Can I help you find what you're looking for?",
    personality: "friendly and helpful",
    primaryColor: "#6366f1",
    allowTopics: "products, pricing, shipping, returns",
    provider: "groq",
  })

  // Load config on mount
  useEffect(() => {
    import('./actions').then(({ getWidgetConfig }) => {
      getWidgetConfig().then(saved => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      else alert('Configuration saved to database!')
    } catch (e) {
      console.error(e)
    } finally {
      setIsSaving(false)
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Aria, Max, Sales Agent"
              />
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {personalities.map(p => (
                  <button key={p.value} onClick={() => setConfig(prev => ({ ...prev, personality: p.value }))}
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap bg-white/[0.02] border border-white/[0.06] p-3 rounded-xl">
                {themes.map(t => (
                  <button key={t.value} onClick={() => setConfig(p => ({ ...p, primaryColor: t.value }))}
                    title={t.label}
